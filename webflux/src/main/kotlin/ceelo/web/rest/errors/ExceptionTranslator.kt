...
class ExceptionTranslator(private val env: Environment) : ProblemHandling, SecurityAdviceTrait {
    ...
    @ExceptionHandler
    fun handleEmailAlreadyUsedException(ex: ceelo.service.EmailAlreadyUsedException, request: ServerWebExchange): Mono<ResponseEntity<Problem>> {
    ...
    @ExceptionHandler
    fun handleUsernameAlreadyUsedException(ex: ceelo.service.UsernameAlreadyUsedException, request: ServerWebExchange): Mono<ResponseEntity<Problem>> {
        val problem = LoginAlreadyUsedException()
        return create(problem, request, HeaderUtil.createFailureAlert(applicationName, true, problem.entityName, problem.errorKey, problem.message))
    }

    @ExceptionHandler
    fun handleInvalidPasswordException(ex: ceelo.service.InvalidPasswordException, request: ServerWebExchange): Mono<ResponseEntity<Problem>> {
    ...
    private fun containsPackageName(message: String?) = listOf("org.", "java.", "net.", "javax.", "com.", "io.", "de.", "ceelo").any { it == message }
}
