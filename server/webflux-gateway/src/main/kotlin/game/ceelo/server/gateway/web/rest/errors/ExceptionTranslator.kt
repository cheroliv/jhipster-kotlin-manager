package game.ceelo.server.gateway.web.rest.errors
...
class ExceptionTranslator(private val env: Environment) : ProblemHandling, SecurityAdviceTrait {
    ...
    @ExceptionHandler
    fun handleEmailAlreadyUsedException(ex: game.ceelo.server.gateway.service.EmailAlreadyUsedException, request: ServerWebExchange): Mono<ResponseEntity<Problem>> {
    ...
    @ExceptionHandler
    fun handleUsernameAlreadyUsedException(ex: game.ceelo.server.gateway.service.UsernameAlreadyUsedException, request: ServerWebExchange): Mono<ResponseEntity<Problem>> {
        val problem = LoginAlreadyUsedException()
        return create(problem, request, HeaderUtil.createFailureAlert(applicationName, true, problem.entityName, problem.errorKey, problem.message))
    }

    @ExceptionHandler
    fun handleInvalidPasswordException(ex: game.ceelo.server.gateway.service.InvalidPasswordException, request: ServerWebExchange): Mono<ResponseEntity<Problem>> {
    ...
    private fun containsPackageName(message: String?) = listOf("org.", "java.", "net.", "javax.", "com.", "io.", "de.", "game.ceelo.server.gateway").any { it == message }
}
